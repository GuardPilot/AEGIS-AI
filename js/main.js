...
(function(){
  const pdfPath = 'assets/GuardPilot_PitchDeck.pdf';
  function ensureDialog(){
    let dlg = document.querySelector('dialog.deck-dialog');
    if (dlg) return dlg;
    dlg = document.createElement('dialog');
    dlg.className = 'deck-dialog';
    dlg.innerHTML = `
      <form method="dialog" class="deck-form" id="deckForm" aria-live="polite">
        <h3 style="margin:0 0 6px">Download Pitch Deck</h3>
        <p class="only-ru" style="margin:0 0 6px">Оставьте контакты, чтобы получить ссылку на презентацию.</p>
        <label>Name / Имя<input name="name" autocomplete="name" /></label>
        <label>Company / Компания<input name="company" autocomplete="organization" /></label>
        <label>Email*<input name="email" type="email" required autocomplete="email" /></label>
        <div class="deck-actions">
          <button type="button" class="btn" id="deckCancel">Cancel</button>
          <button type="submit" class="btn btn--primary" id="deckSubmit">Send</button>
        </div>
        <div class="status" id="deckStatus" role="status" aria-live="polite"></div>
      </form>`;
    document.body.appendChild(dlg);
    dlg.querySelector('#deckCancel').addEventListener('click', ()=> dlg.close());
    dlg.addEventListener('close', ()=>{
      const f = dlg.querySelector('#deckForm'); if (f) f.reset();
      dlg.querySelector('#deckStatus').textContent='';
    });
    dlg.querySelector('#deckForm').addEventListener('submit', async (e)=>{
      e.preventDefault();
      const fd = new FormData(e.currentTarget);
      const name = (fd.get('name')||'').toString().trim();
      const company = (fd.get('company')||'').toString().trim();
      const email = (fd.get('email')||'').toString().trim();
      const status = dlg.querySelector('#deckStatus');
      status.textContent = 'Checking file availability...';
      let available = false;
      try {
      ...
      if (available){
        status.innerHTML = `<a href="${pdfPath}" class="btn btn--primary" style="text-decoration:none" download>Download PDF</a>`;
      } else {
        status.textContent = `We’ll send you the deck soon`;
        const mto = `mailto:2025pilot.guard@gmail.com?subject=Pitch%20Deck%20Request&body=${encodeURIComponent(
          `Name: ${name}\nCompany: ${company}\nEmail: ${email}\n——\nPlease send the deck link.`
        )}`;
        window.location.href = mto;
      }
    });
    return dlg;
  }
  document.addEventListener('click', (e)=>{
    const t = e.target.closest('[data-action="open-deck"]');
    if (!t) return;
    e.preventDefault();
